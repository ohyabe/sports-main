<template>
  <div class="container">
    ...
    <section class="questions">
      <div v-for="question in sportQuestions" :key="question.id" class="question-box">
        ...
        <p><strong>提交時間:</strong> {{ question.issueReportTime.join('-') }}</p>
        <p><strong>老師名字:</strong> {{ question.teacherName || "尚未回覆" }}</p>
        <p><strong>老師回覆:</strong> {{ question.teacherResponse || "尚未回覆" }}</p>
        <p><strong>回覆時間:</strong> {{ question.issueResponseTime || "尚未回覆" }}</p>
        
        <div class="button-container">
          <button @click="toggleResponseInput(question.id)">
            {{ question.teacherResponse ? "更新回復" : "回覆問題" }}
          </button>
        </div>
        
        <div v-if="question.showResponseInput" class="response-input">
          <textarea v-model="question.teacherResponse" placeholder="輸入您的回覆"></textarea>
  ...
  methods: {
    fetchSportQuestions() {
      fetch('http://localhost:8080/question/all')
        ...
        .then(data => {
          this.sportQuestions = data.filter(question => question.issueType === "賽事").map(question => {
            // 檢查 issueReportTime 是否為陣列，如果不是則轉換為陣列
    ...
    toggleResponseInput(questionId) {
      const question = this.sportQuestions.find(q => q.id === questionId);
